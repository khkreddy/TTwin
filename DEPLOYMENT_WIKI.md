...
2. **Learn-by-solve** under `assessment.learn_by_solve`: a hinge solve plus one follow-up MCQ for **every wrong A–D option** when `mcq_key` is available. Each follow-up is an **option-specific unlocking question** about that option’s content (approved gold: `9701_m16_qp_12:q1` — B = OH oxidation states in Mg(OH)₂, C = S in Na₂SO₄, D = Cl in NH₄Cl; spectroscopy banks are the same pattern). Generic/template stems (`Which check is required?`, `Which claim does the item actually require?`, `What did they drop?`, `The keyed choice is …`, `This option claims … Is that true for the situation in the stem?`, `fit this requirement`, `correctly answer this`, `A working produced … Which combination of the stem quantities`, `what this question is asking for`, `is what the stem asks for`, `Which operation on the stem data`, `What is true of … in the situation the stem describes?`) are **not complete**. A follow-up is also a stamp when it has **wrapper shape**: it treats the original MCQ as the criterion (`the stem`, `this question`, `the situation … described`, `asking for`, `holds for`, `apply here`) or Yes/No polarity about whether the quoted option holds. Follow-up options that only say the choice “is exactly what is required” / “does not meet that requirement” / “is what the stem asks for” / “does not hold for the situation” are stamps. Runtime is a JSON lookup. V2 mx types (`term_substitution` · `condition_omission` · `relationship_reversal` · `scope_error` · `surface_feature_capture` · `mechanism_conflation` · `operation_confusion`) sit on the teacher key only — never on the learner paper.
