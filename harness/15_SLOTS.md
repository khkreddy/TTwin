...
| T-MOD | compiled `modify_packet.v1` (`83_MODIFY.md`): whitelist source + one hinge row + seeds + instruction ≤1k. Not the thin stem+A–D packet. | whole item of the declared `item_type` (not only four-option MCQ), session namespace, key UNVERIFIED | new freeze uid, pool membership, partial-item edits, printing mx types on the learner stem, map dump | Invalid → keep original item |
