...
| T-MOD | one item, learner-whitelist fields + instruction ≤1k (instruction may be a packed **modify seed** from the Mx library) | whole item (stem + A–D; TikZ only if needed), session namespace, key UNVERIFIED | new freeze uid, pool membership, partial-item edits, printing mx types on the learner stem | Invalid → keep original item |
