...
- **Complete-question overlay** at pack time (`tools/lbs_construct.py` + `tools/join_lbs.py`). Does not rewrite freeze exam.v1. A complete packed item keeps five-click tags, `assessment.learn_by_solve` (one follow-up MCQ per wrong option when a key exists), extracted examiner comments where available, and `assessment.modify_seeds` from the Mx V2 library for T-MOD. Mix-up type names are teacher-key only.
- **Learner field whitelist.** Stem, options / option-table, figures (TikZ / original crop), equations, statements, tables, and (student-take only) the learn-by-solve *follow-up question text and its why* after a wrong letter. After the hint, retry the original; do not print the original key on the hint. **Banned on learner copy:** mx ledger, mx_type labels, examiner comments, crop paths, printed SMILES strings, mix-up ledgers, original answer keys on the hint, modify seeds. SMILES may be packed for the drawer; they are never printed as text.
