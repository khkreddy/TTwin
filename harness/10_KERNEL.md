...
- **Complete-question overlay** at pack time (`tools/lbs_construct.py` + `tools/join_lbs.py`). Does not rewrite freeze exam.v1. A complete packed item keeps five-click tags, `assessment.learn_by_solve` (one follow-up per wrong option when a key exists; format fits the gate), extracted examiner comments where available, and `assessment.modify_seeds` from the Mx V2 library for T-MOD. Mix-up type names are teacher-key only.
