...
- **Complete-question overlay** at pack time (`tools/lbs_compile.py` instantiate + `tools/join_lbs.py`; `lbs_construct.py` is legacy). Does not rewrite freeze exam.v1. Hint recipes compile from map units × mx/steps; packed items instantiate with **zero** model calls (`82_LBS_COMPILER.md`). A complete packed item keeps five-click tags, `assessment.learn_by_solve` (one follow-up per wrong option when a key exists; format fits the gate), extracted examiner comments where available, and `assessment.modify_seeds` from the Mx V2 library for T-MOD. Mix-up type names are teacher-key only.
- **Learner field whitelist.** Stem, options / option-table, figures (TikZ / original crop), equations, statements, tables, and (student-take only) the learn-by-solve *follow-up question text and its why* after a wrong letter. Submitting the hint returns the student to the original with that letter cleared; do not print the original key on the hint. **Banned on learner copy:** mx ledger, mx_type labels, examiner comments, crop paths, printed SMILES strings, mix-up ledgers, original answer keys on the hint, modify seeds. SMILES may be packed for the drawer; they are never printed as text.
